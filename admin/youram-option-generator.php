...
class YouRamOptionGenerator {
	...
	protected $points;
	var $youram_instance_name = '';
	var $youram_option_name = '';
	var $youram_allowed_instances = array(
        0 => "preview", 
		1 => "single-video",
		2 => "single-video-subscribe",
		3 => "single-video-color",
		4 => "gallery-list-view",
		5 => "gallery-grid-view",
		6 => "gallery-popup",
		7 => "gallery-4columns",
		8 => "gallery-color",
		9 => "gallery-only-images",
		10 => "gallery-cards",
		11 => "gallery-with-tabs",
		12 => "gallery-tabs-cards",
    );
	...
	public function __construct( $pages = array() ) {
		...
		//error_log("CONSTRUCT:".$_GET['page']."--".$_GET['instance']."--".$this->youram_option_name);

		if(isset($_GET['page']) && $_GET['page']=="youram") {
			$this->youram_instance_name = isset( $_GET['instance'] ) ? $_GET['instance'] : 'preview';
			$this->youram_option_name = 'youram_'.$this->youram_instance_name;
			update_option('youram_current_instance',$this->youram_option_name);
		} else {
			$this->youram_option_name = get_option('youram_current_instance');
		}
		...
		//error_log("PAGES:".print_r(count($this->pages),true));
	...
	public function admin_init() {
		...
		foreach ( $this->pages as $page_key => $page_params ) {
			...
			//error_log("REGISTER_SETTING:".$this->youram_option_name);
			register_setting($page_key,$this->youram_option_name,$page_params['sanitize']);
			
			//error_log('SANTIZE:'.print_r($page_params['sanitize'],true));
			...
			if ( !empty( $page_params['sections'] ) ) {
				foreach ( $page_params['sections'] as $section_key => $section_params ) {
					...
					//error_log('SECTION ADD:'.print_r($params,true));
					//call_user_func_array( 'add_settings_section', $params );
					add_settings_section($params['id'],$params['title'],$params['callback'],$params['page']);
					...
					if ( !empty( $section_params['fields'] ) ) {
						foreach ( $section_params['fields'] as $field_key => $field_params ) {
							...
							add_settings_field($params['id'],$params['title'],$params['callback'],$params['page'],$params['section']);
	...
	protected function build_menu_page( $page_key ) {
		...
		$this->options = get_option($this->youram_option_name, array());
		?><div class="wrap" style="text-align: center;">
		<table class="youram-admin-table"><tr><td>
		<div class="youram-admin">
			<h1><?php echo $GLOBALS['title']; ?></h1>
			<p class="youram-page-sub-title">{ YouTube | Embed | Gallery }</p>
			<p id="youram-admin-loading-image"></p>
			<p class="youram-page-sub-title-instruction"><strong><!--1.Save all settings below. &nbsp;&nbsp; 2.Add Shortcode &#91;youram&#93; on your page. &nbsp;&nbsp; 3.Use "Rebuild Cache" button on Youram frontend if required.--></strong></p>
			<table class='youram-admin-header'><tr>
			<td>
				<div class='youram-admin-instance-block'>
					<i class="fas fa-bars"></i>&nbsp;
					<p>YouRam Instance:</p>
					<select class="" id="youram-admin-instance-select">
					</select>
					
				</div>
			</td></tr>
			<tr><td>
				<div class='youram-admin-shortcode'><i class='fas fa-code'></i> Shortcode: [youram instance='<?php echo $this->youram_instance_name; ?>']</div>
			</td>
			</tr></table>
			<!--<a href=".">Youram2</a>-->
			...
			if ( !empty( $page['sections'] ) ) {
				...
				<div id="youram-admin-option-tabs"></div>
				<?php
				...
				?>
				<table class="youram-admin-submit-table"><tr>
				<td><button id="youram-admin-preview"><i class="fa fa-sync-alt"></i>&nbsp;Re-Build Cache</button></td>
				<td style="text-align: right;"><div id="youram-admin-submit-loading-image"></div><?php
					if ( $this->has_fields( $page ) ) {
						submit_button();
					}
				?></td>
				<tr></tr><tr>
				<td><p id='youram-admin-options-change-notice' class='youram-admin-notice'></p></td>
				<td><p id="youram-premium-notice" class='youram-admin-notice'><i class="fa fa-info-circle"></i> &nbsp;Options that are disabled <br>are Premium options..</p></td>
				</tr></table>
				</form>
				
				
				<table class='youram-admin-feedback-table'><tr>
				<td><input class="regular-text" type="text" id="youram-admin-feedback-input" placeholder="Need Help? Any Question / Bug / Feature?"></td>
				<td><button id="youram-admin-feedback-ask" class="button button-primary">Ask Now</button></td>
				</tr></table>
				<script>
				var youramAskUser = '<?php echo wp_get_current_user()->user_email ?>';
				var youramSelectedInstance = '<?php echo $this->youram_instance_name ?>';
				var youramAllowedInstances = '<?php echo json_encode($this->youram_allowed_instances) ?>';
				</script>
				<br>
				<p class='codeisart' title='feed animals..'><i class="fas fa-paw"></i> feed animals around you.. <i class="fas fa-heart"></i></p>
				<p class='codeisart-content'></p>
		...
		?></div>
			...
			<div id='youram-preview-control-box' class=''>
				<div id='youram-preview-control-tv' class='youram-preview-control'><i class='fa fa-tv'></i></div>
				<div id='youram-preview-control-desktop' class='youram-preview-control'><i class='fa fa-desktop'></i></div>
				<div id='youram-preview-control-laptop' class='youram-preview-control'><i class='fa fa-laptop'></i></div>
				<div id='youram-preview-control-tablet' class='youram-preview-control'><i class='fa fa-tablet-alt'></i></div>
				<!--<div id='youram-preview-control-mobile' class='youram-preview-control'><i class='fa fa-mobile-alt'></i></div>-->
			</div>
			<div id='<?php echo $this->youram_option_name; ?>' class='youram'></div>
			
			<!--<div id='youram-feedback'><a href="https://wordpress.org/support/plugin/youram-youtube-embed/" target="_blank">Wp.org Forum Support ></a></div>-->
	...
	protected function build_settings_field( $page_key, $section_key, $field_key ) {
		...
		switch ( $field['type'] ) {
			case 'button':
				printf(
					'<button %s id="%s" name="%s" title="%s" type="button">%s</button> &nbsp; %s',
					!empty( $field['class'] ) ? "class='{$field['class']}'" : '',						// class
					$field['id'],																		// id
					"{$this->youram_option_name}[{$field['id']}]",											// name
			...
			case 'notice':
				printf(
					'<div %s id="%s" name="%s" title="%s" type="button">%s</div> &nbsp; %s',
					!empty( $field['class'] ) ? "class='{$field['class']}'" : '',						// class
					$field['id'],																		// id
					"{$this->youram_option_name}[{$field['id']}]",											// name
					$field['title_attr'],																// title
			...
			case 'select':
                                if (!empty($field['attributes']) && isset($field['attributes']['multiple']) && $field['attributes']['multiple']) {
                                  $field_tag_name = "{$this->youram_option_name}[{$field['id']}][]";
                                  $field_name = "{$field['id']}[]";
                                }
                                else {
                                  $field_tag_name = "{$this->youram_option_name}[{$field['id']}]";
				...
				foreach ( $field['choices'] as $value => $text ) {
					...
					if ( isset( $this->options[ $field['id'] ] ) ) {
						...
						if (!is_array($this->options[ $field['id'] ] ) ) {
						  //printf("Selected:%s",$this->options[ $field['id'] ]);
						  $selected = $value == $this->options[ $field['id'] ] ? 'selected="selected"' : '';
						}else {
							//multi-valued select options
						  $selected = '';
			...
			case 'textarea':
				...
				break;
			case 'wp_editor':
				$field['textarea_name'] = "{$this->youram_option_name}[{$field['id']}]";
